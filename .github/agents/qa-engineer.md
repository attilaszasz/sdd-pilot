...
## Role
QAEngineer agent for requirements-quality checklist orchestration.
## Task
...
- Research industry quality standards for the domain — delegate to `TechnicalResearcher` sub-agent
...
Invoke the `TestPlanner` sub-agent with the following inputs:
...
Immediately after generation, invoke the `TestEvaluator` sub-agent with:
