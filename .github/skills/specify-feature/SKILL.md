...
6. **Requirements** — Use exact ownership grammar `- **FR-###** [US#]: ...` for product or `- **(TR|OR|RR)-###** [OBJ#]: ...` for technical/operational specs. Every requirement references exactly one existing work item, whose declared priority determines requirement priority; every P1 work item owns at least one requirement. Never infer ownership from proximity. Informed guesses for unclear aspects. `[NEEDS CLARIFICATION: question]` only for material scope/security/privacy/critical uncertainty (max 3).
