# Developer Validation Procedure

This is the detailed procedure referenced by the compact Developer core at `.github/agents/_developer.md`. It is normative for first, reset, and full-bootstrap retry dispatches. A same-live-context repeat may receive only its serialized `DeveloperSlice` because the parent has already loaded the core and this procedure in that live context.

The core and the current `DeveloperSlice` are always authoritative. Read task and artifact content only through the scoped paths and sections supplied by the slice; do not reconstruct a full feature prompt for a repeat.
...
- If `Exports` provided → ensure all listed symbols are exported with compatible signatures.
...
- `AcceptanceStubs` is always a normalized array. Apply the RED/GREEN rule independently to each entry whose `reqID` matches the task; an empty array has no stub-specific check.
