...
- [ ] On the first FAIL: `Status: FAILURE`, `errorType: export-contract`, `affectedFile` = task `FilePath`, `affectedLine` = declaration line when determinable; the task is NOT marked complete and routes into the orchestrator's error-recovery loop.
