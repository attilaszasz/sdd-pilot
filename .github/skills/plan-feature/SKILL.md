...
5. Read the live `FEATURE_DIR/checklists/.checklists` when present. Preserve every existing `CHL###` line and checkbox state. Match detected domains case-insensitively to existing entries; append only new domains using IDs above the highest existing `CHL###`. Never reorder or reset entries.
6. If the queue is absent, write `FEATURE_DIR/checklists/.checklists`:
   ```
...
7. `.checklists` exists → merge as described above. **Autopilot guard (P6)** logs Outcome="Merge"; it never overwrites or resets the queue. Interactive destructive migration requires the ambient explicit mapping and validation procedure.
