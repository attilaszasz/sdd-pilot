...
1. Run `node .github/sddp/workflows/quality-control/scripts/parse-review-findings.mjs "FEATURE_DIR/.review-findings"` from the repository root. Require exit zero, `valid: true`, `version: 1`, and an empty `errors` array.
2. The parser resolves every non-empty path under the canonical repository root. Missing paths, control characters, aliases, and symlink escapes are invalid and fail closed. On parser failure, malformed JSON, unsupported version/type, or invalid fields: report the parser errors and stop with **BLOCKED** before Story Verifier delegation, report generation, or BUG task generation. Never reinterpret unversioned pipe records or partially use valid lines from an invalid file.
...
7. When the helper reports `written: true`, Delete `FEATURE_DIR/.completed` and `FEATURE_DIR/.qc-passed` and append new bug context to the report. A valid dedupe no-op still deletes those markers because the underlying failure remains active:
    ...
    ```
8. Write `## Bug Context` in `qc-report.md`: bug task ID → full error output, stack trace frames, related test.
9. Report: "QC failed. Added [N] bug tasks ([X] CRITICAL, [Y] ERROR, [Z] WARNING). Removed `.completed`."
...
4. Atomically create `.qc-passed` in the same directory: write and flush `.qc-passed.tmp`, containing exactly `QC Passed: <current ISO 8601 timestamp>`, `QC Baseline Commit: <BASELINE_COMMIT>`, `QC Repository State SHA-256: <REPOSITORY_STATE_SHA256>`, `QC Report SHA-256: <REPORT_SHA256>`, and `QC Evidence SHA-256: <EVIDENCE_SHA256>` on separate lines; rename it to `.qc-passed`; remove the temp file on error. Never expose a partial marker.
5. Before writing the marker, record `QC Baseline Commit: <current 40-character HEAD>` and `QC Repository State SHA-256: <digest>` alongside the existing fields. The state digest is SHA-256 of `git status --porcelain=v1 -z --untracked-files=all`, excluding this feature's `.completed`, `.qc-passed`, and `qc-report.md` records. Git metadata or HEAD unavailable → BLOCKED; a fresh full QC remains permitted. This captures all tracked, staged, renamed, deleted, and untracked repository changes that can affect the build, test, or runtime.
6. Immediately validate the marker by recomputing the report, evidence, baseline, and repository-state digests. Any mismatch deletes `.qc-passed` and changes the outcome to BLOCKED.
