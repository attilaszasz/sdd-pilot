...
- Check if git repo → run `node scripts/ensure-implement-state-ignored.mjs "$PROJECT_ROOT"` before any `.implement-state` checkpoint. This appends `.implement-state` once without replacing existing ignore bytes; failure blocks checkpointing.
