...
- Only halt for: (1) Gate auto-resolution failed, (2) self-healing amendment or post-amendment gate failure, (3) Sequential task failed after retry + (`AUTOPILOT = true` or user chooses Halt), (4) unrecovered Phase Review or Micro-QC failure at final validation, (5) All tasks already complete
...
- **Self-healing artifact updates**: When the Developer reports a `Divergence` (Section 3.6 of `_developer.md`), read and execute `references/self-healing-amendments.md` immediately after the divergent task succeeds and before processing the next task. Record every changed artifact, revalidate amended phase inputs, then re-parse `COVERAGE_MATRIX` from the validated `plan.md` so the next task's `ExpectedEvidence` and the Phase Review coverage diff use fresh values. Preserve all cross-referenced IDs (Req IDs, task IDs, `AD-###` IDs, `ADR-NNNN`); only cell values and new feature-local `AD-###` rows may change. Log every amendment to `FEATURE_DIR/divergence-log.md`. A divergence is a SUCCESS signal only after its amendments and mandatory post-amendment gates pass; an amendment or gate failure leaves the task unchecked and blocks implementation.
...
- Optional `P1_REQUIREMENT_SNAPSHOT` input: retain it separately from `PIPELINE_CONTEXT` and pass it to `references/gates.md` on every invocation. The gate validates the current `spec.md` checksum before passing P1 IDs to downstream validators; standalone and stale-snapshot runs use live parsing.
...
> Executed via `references/gates.md` on every invocation (Step 1). Project setup is idempotent; resume never skips validation.
...
4. If the Developer reported one or more `Divergence` blocks, read and execute `references/self-healing-amendments.md`. Do not load that reference when no divergence was reported. A self-healing amendment or post-amendment gate failure overrides this SUCCESS: remove the task from `IN_REVIEW_TASKS`, keep its checkbox `[ ]`, and halt before another task, Phase Review, task completion, or `.completed` creation.
...
6. **TENTATIVE_TASKS handoff to QC**: if `TENTATIVE_TASKS` is non-empty, write version 1 canonical JSON Lines to `FEATURE_DIR/.review-findings`, one object per tentative task with exactly these fields in this order and no extra whitespace: `{"version":1,"task":"T###","requirements":["FR-001"],"type":"tentative","evidence":"one-line evidence","paths":["repository/relative/path"]}`. Populate `requirements` from all task requirement tags and `paths` from all known task file paths; either array may be empty. Before appending, run `node .github/sddp/workflows/quality-control/scripts/parse-review-findings.mjs "FEATURE_DIR/.review-findings"` when the file already exists and halt without changing it if validation fails. Append only records not already present with the same canonical JSON, then run the parser again; a non-zero exit or `valid: false` is an implementation blocker and prevents `.completed`. Unversioned pipe records are intentionally incompatible and must be reported for manual cleanup, never inferred or migrated. Keep valid records through QC reruns; they are evidence inputs. Remove the file only when its Feature Workspace is archived or deleted. These become QC priority-review checks (Story Verifier `priorityChecks`). Report: "⚠ [N] tentative task(s) flagged for QC priority review: [task IDs]."
