...
If `PIPELINE_CONTEXT` is absent or invalid, **Delegate: Context Gatherer** (`.github/agents/_context-gatherer.md`) (quick mode) with `autopilot=false` → resolve `FEATURE_DIR`.
...
WHILE ITERATION < MAX_ITERATIONS:
    ...
    **Escalation rules** (per BUG task):
    ...
    Unless `PENDING_END_REASON` is set, load+execute `.github/sddp/workflows/implement-tasks/WORKFLOW.md` (full workflow), passing `PIPELINE_CONTEXT` unchanged and forwarding `P1_REQUIREMENT_SNAPSHOT` separately.
    ...
    When no `PENDING_END_REASON` is set, load+execute `.github/sddp/workflows/quality-control/WORKFLOW.md` (full workflow), passing `PIPELINE_CONTEXT` unchanged. Do not pass `P1_REQUIREMENT_SNAPSHOT` to QC; it is only a validator input optimization.
