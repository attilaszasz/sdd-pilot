...
- Optional `SKIP_CHECKLIST_QUEUE` input: only `/sddp-autopilot` sets this from an epic `skip_checklist` hint. It suppresses new queue generation but never bypasses existing checklist state.
</rules>
...
- `plan.md` exists:
  - Snapshot existing `AD-###` IDs, coverage/stub requirement references, downstream tasks, checklist paths, and checked lines.
  - **Autopilot guard (P1)**: `AUTOPILOT = true` → choose Refine, log a `decision` row to `FEATURE_DIR/autopilot-log.md`: Timestamp=now, Phase=`Plan`, Event=`decision`, Detail="Existing plan.md found", Outcome="Refine", Rationale="preserve IDs and downstream state", Artifacts=`[plan.md](plan.md)`, then update the existing plan in place.
  - `AUTOPILOT = false` → ask "Refine or Destructive migration?". Refine preserves existing content and IDs while updating it in place. Destructive migration follows the ambient interactive-only mapping, atomic downstream-update, and validation procedure; failure leaves original bytes unchanged.
  - Before accepting refinement, verify every baseline `AD-###`, requirement reference, checklist path, and checked line remains and all existing downstream task references still resolve. Failure restores the original plan bytes and halts.
...
1. If `SKIP_CHECKLIST_QUEUE = true`, run `node scripts/checklist-state.mjs "FEATURE_DIR"` from the repository root before analyzing risk signals:
   - `overallStatus = "N/A"` or `"PASS"` → do not create, append to, or otherwise mutate `checklists/` or `.checklists`. Report that queue generation was skipped by the Autopilot hint.
   - Any other, malformed, or unreadable result → **HALT**. Existing incomplete checklist state remains authoritative. With `AUTOPILOT = true`, log a `halt` row: Phase=`Plan`, Event=`halt`, Detail="Checklist skip conflict: existing checklist state blocks", Outcome="Halt planning", Rationale="skip_checklist cannot bypass an existing pending or malformed checklist", Artifacts=`[checklists/](checklists/)`. Do not create or modify `plan.md` further, the queue, or checklist files.
2. Read the stable `MAX_CHECKLIST_COUNT` from `PIPELINE_CONTEXT` or the current Context Report. `0` → skip entirely.
3. Analyze `plan.md`, `spec.md`, design artifacts for risk/domain signals:
...
4. Rank by signal strength, select top N = min(detected, `MAX_CHECKLIST_COUNT`)
5. Ensure `FEATURE_DIR/checklists/` exists.
6. Read the live `FEATURE_DIR/checklists/.checklists` when present. Preserve every existing `CHL###` line and checkbox state. Match detected domains case-insensitively to existing entries; append only new domains using IDs above the highest existing `CHL###`. Never reorder or reset entries.
7. If the queue is absent, write `FEATURE_DIR/checklists/.checklists`:
   ```
...
8. `.checklists` exists → merge as described above. **Autopilot guard (P6)** logs Outcome="Merge"; it never overwrites or resets the queue. Interactive destructive migration requires the ambient explicit mapping and validation procedure.
