...
| **Plan** | `/sddp-plan` | `plan.md`, `research.md`, conditionally `data-model.md`, `contracts/` | `spec.md` exists + **Spec Validator** PASS (≤3 unresolved markers, no unresolved CRITICAL/HIGH stress-test findings, concrete P1 acceptance criteria, frontmatter complete) |
...
- **Spec → Plan** — `/sddp-plan` Step 1.6 delegates the Spec Validator (`_spec-validator.md`) against `spec.md`: enforces ≤3 unresolved `[NEEDS CLARIFICATION]` markers, no unresolved CRITICAL/HIGH stress-test findings (marker-cap deferral is not a waiver), concrete acceptance criteria for every P1 user story or objective, and frontmatter completeness (`spec_type`, `spec_maturity`).
...
- **Tasks → Implement** — `/sddp-implement` (via `references/gates.md`) delegates the Tasks Validator (`_tasks-validator.md`) against `tasks.md` (with `spec.md` for P1 IDs): enforces complete deterministic parsing with line-numbered errors, ≤40 tasks, every P1 requirement has ≥1 task, no circular `after:T###` chains (static graph cycle check), `tasks.md` ≤ 6 KB, and valid phase structure (Setup → Foundational → Delivery → Polish, no empty optional phases, unique sequential `T###` IDs).
