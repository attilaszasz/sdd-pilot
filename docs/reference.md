...
After Clarify completes or is skipped, autopilot creates a separate ephemeral `P1_REQUIREMENT_SNAPSHOT` from the current `spec.md`. It contains the ordered P1 requirement IDs and a SHA-256 digest of the exact file bytes. The Plan → Tasks and fresh Tasks → Implement callers compare that digest with the live `spec.md` before passing `P1RequirementIds` to their validators. Missing, malformed, unreadable, or mismatched snapshots fall back to the validators' normal live parsing; the validator is never skipped and no verdict or marker is cached. Standalone commands do not create the snapshot.
